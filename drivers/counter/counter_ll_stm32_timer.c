				...
				const struct counter_alarm_cfg *alarm_cfg)
{
	 ...
	 */
	diff = counter_stm32_ticks_sub(val - 1U, read(dev), top);
	if (diff > max_rel_val) {
		if (absolute) {
			err = -ETIME;
		}
...
static int counter_stm32_init_timer(const struct device *dev)
{
	struct counter_stm32_config *cfg =
		(struct counter_stm32_config *)DEV_CFG(dev);
	...
	cfg->info.freq = tim_clk / (cfg->prescaler + 1U);
...
#define COUNTER_DEVICE_INIT(idx)						  \
	BUILD_ASSERT(DT_INST_PROP(idx, st_prescaler) <=				  \
		     0xFFFF,							  \
		     "TIMER counter prescaler out of range");			  \
	...
	static struct counter_stm32_ch_data counter##idx##_ch_data[TIMER_MAX_CH]; \
										  \
	static int counter##idx##_init(const struct device *dev)		  \
	{									  \
		TIMER_IRQ_CONNECT(idx);						  \
		return counter_stm32_init_timer(dev);				  \
	}									  \
										  \
	static struct counter_stm32_config counter##idx##_config = {		  \
		.info = {							  \
			.max_top_value =					  \
		...
		.timer = TIM(idx),						  \
		.prescaler = DT_INST_PROP(idx, st_prescaler),			  \
		.pclken = TIMER_CLK(idx),					  \
